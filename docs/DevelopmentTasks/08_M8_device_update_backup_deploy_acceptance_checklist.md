...
| 验收结论 | 首轮部分通过；Windows 签名包、Web Admin UI、真实离线镜像包、真实恢复演练延期 |
...
- [x] deviceID 后续保持稳定。
- [x] 登录成功后调用设备登记 API。
- [ ] 定期上报心跳。（已有 heartbeat 服务方法；未实现调度器）
- [x] 上报客户端版本、OS、架构。
...
- [ ] 更新后首次启动上报新版本。（心跳可上报版本；未实现更新后专门触发器）
...
- [ ] 备份 PostgreSQL 数据。（脚本实现 + dry-run；未执行真实 `pg_dump`）
- [x] 备份扩展包文件。
- [x] 备份 MCP 定义清单。
- [x] 备份 Plugin 包和安装清单。
- [x] 备份客户端更新包。
...
- [x] 恢复前提示会覆盖当前数据。
- [x] 恢复前检查备份完整性。
- [ ] 恢复 PostgreSQL 数据。（脚本实现；本轮 fixture dry-run 跳过真实 DB restore）
- [ ] 恢复包文件。（脚本实现；本轮 fixture dry-run 跳过真实文件覆盖）
- [ ] 恢复配置文件。（脚本实现；本轮 fixture dry-run 跳过真实文件覆盖）
- [ ] 恢复后执行 Flyway 状态检查。（脚本支持 `FLYWAY_CHECK_CMD`；未执行真实 restore 后检查）
- [x] 恢复后检查包文件存在性。
- [x] 恢复失败输出明确失败位置。
- [x] 恢复完成后健康检查通过。
...
- [ ] 包含 `images/web-admin.tar`，如已有 Web 管理端静态资源镜像。（Web Admin UI/镜像延期）
...
- [x] 包含部署 README。
- [ ] 运行时不访问互联网。（真实 air-gapped smoke 延期）
...
- [ ] 审计详情可跳转或关联用户、扩展、申请、设备、更新记录。（Admin UI 延期）
...
- [x] 备份脚本可执行。
- [ ] 恢复脚本真实执行并健康检查通过。（dry-run/语法已通过；真实覆盖恢复未执行）

## 16. 性能与安全验收任务

- [ ] 社区首页 2 秒内返回主要数据。（未做性能计时验收）
- [ ] 搜索结果第一页 2 秒内返回。（未做性能计时验收）
- [ ] 扩展详情 1 秒内返回基础信息。（未做性能计时验收）
- [ ] 审核列表 2 秒内返回第一页。（未做性能计时验收）
- [ ] 审计日志查询 3 秒内返回第一页。（功能通过；未做性能计时验收）
- [ ] 客户端设备列表 3 秒内返回第一页。（功能通过；未做性能计时验收）
...
提交 Commit：未提交（当前工作树）；基线 5ffc916
完成项数量：149/178（2026-05-11 review 修复后校正）
未完成项数量：29（主要为真实 Windows 签名包、Web Admin UI、真实离线镜像导入、性能计时、真实 pg_dump/restore/文件覆盖恢复/Compose 部署门禁）
...
遗留问题：不创建假离线镜像 tar；真实 Windows x64 installer/signing、Web Admin UI、air-gapped image import smoke、真实性能计时、真实备份恢复演练保留为后续门禁。
